...
6. **figure_notes** — **常に文字列で必須(null不使用=ゴールド流儀・2026-08-12まるこ裁可)**。
   図あり=値域と描画可能性・ラベル分離の設計記録。図なし=「図なし」で始まる理由文
   (例:「図なし(位取り操作・数値のみ)。」)。整合は `tests/rationale_integrity.js` が悉皆検査
   (バンクの figure_params 有無と figure_notes 内容の対応)。
   ※既存の jhs各章・g01 は旧 null 流儀のまま。次に触るとき順次この正準へ統一(md5非依存・一括改修不要)。
