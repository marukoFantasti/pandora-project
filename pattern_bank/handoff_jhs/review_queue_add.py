...
種類=問題文(新パターンid)／図(新kind・描画変更+目視ポイント)／読み(1字=字ごと・多字語=便単位で語リスト)。状態=未/済/×。見る人=まるこ/アイ。配布=配布済(v8)/配布済(v4)/未配布(Fableのシート生成は『未・未配布』からの差分)。"""
...
def add(kind, target, check, by, batch, status='未', dist='未配布'):
    ...
    e = {"id": f"rq-{n:04d}", "種類": kind, "対象": target, "見るべきこと": check, "状態": status, "見る人": by, "追加した便": batch, "追加日": datetime.date.today().isoformat(), "配布": dist}
    q['entries'].append(e); save(q); return e['id']
if __name__ == '__main__':
    ap = argparse.ArgumentParser(); ap.add_argument('--kind'); ap.add_argument('--target'); ap.add_argument('--check'); ap.add_argument('--by', default='アイ'); ap.add_argument('--batch'); ap.add_argument('--set-status', nargs=2, metavar=('ID', 'STATUS')); ap.add_argument('--dist', default='未配布', help='配布済(v8)/配布済(v4)/未配布')
    a = ap.parse_args()
    if a.set_status:
