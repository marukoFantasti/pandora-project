...
function createEngine(lexicon, counterTable) {
  ...
  function scan(text, emit) {
    ...
    while (i < s.length) {
      ...
      if (m) {
        ...
        for (const u of UNIT_KEYS) { if (s.slice(j, j + u.length) === u) { unit = u; break; } }
        if (unit) { const e = entryOf(unit); const hira = Number.isInteger(+numStr) ? readCounter(+numStr, unit) : readNumericLiteral(numStr) + e.base; const orig = numStr + unit; emit(orig, hira, groupRuby(orig, hira)); i = j + unit.length; continue; }
  ...
  function toHiragana(text) { let o = ''; scan(text, (orig, hira) => { o += hira; }); return o; }
  function toRuby(text) { let o = ''; scan(text, (orig, hira, ruby) => { o += ruby; }); return o; }
