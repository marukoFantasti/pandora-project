...
def add(kind, target, check, by, batch, status='未', dist='未配布', subject='算数'):
    ...
    n = 1
    while f"rq-{n:04d}" in ids: n += 1
    e = {"id": f"rq-{n:04d}", "教科": subject, "種類": kind, "対象": target, "見るべきこと": check, "状態": status, "見る人": by, "追加した便": batch, "追加日": datetime.date.today().isoformat(), "配布": dist, "配布日": ""}
    q['entries'].append(e); save(q); return e['id']
if __name__ == '__main__':
    ...
    if not (a.kind and a.target and a.check and a.batch and a.subject): sys.exit('教科(--subject 算数|国語)を含む必須引数が不足\n' + ap.format_help())
    print('added', add(a.kind, a.target, a.check, a.by, a.batch, dist=a.dist, subject=a.subject))
