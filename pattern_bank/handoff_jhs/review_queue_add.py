...
種類=問題文(新パターンid)／図(新kind・描画変更+目視ポイント)／読み(辞書パッチ語+当たり文脈)。状態=未/済/×。見る人=まるこ/アイ。"""
...
def add(kind, target, check, by, batch, status='未'):
    ...
    e = {"id": f"rq-{n:04d}", "種類": kind, "対象": target, "見るべきこと": check, "状態": status, "見る人": by, "追加した便": batch, "追加日": datetime.date.today().isoformat()}
    q['entries'].append(e); save(q); return e['id']
if __name__ == '__main__':
    ap = argparse.ArgumentParser(); ap.add_argument('--kind'); ap.add_argument('--target'); ap.add_argument('--check'); ap.add_argument('--by', default='アイ'); ap.add_argument('--batch'); ap.add_argument('--set-status', nargs=2, metavar=('ID', 'STATUS'))
    a = ap.parse_args()
    if a.set_status:
    ...
    print('added', add(a.kind, a.target, a.check, a.by, a.batch))
