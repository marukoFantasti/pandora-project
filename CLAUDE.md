...
`tests/fixtures/review_queue.json` is the human-review ledger. Every §3-3 / §5 report MUST append entries via `python3 pattern_bank/handoff_jhs/review_queue_add.py --subject 算数|国語 --kind 問題文|図|読み --target <id/kind/word> --check "<what to look at>" --by まるこ|アイ --batch <便名>`: new pattern ids (問題文), new kinds or drawing changes with the 目視 point (図), and furigana patches with their contexts (読み). Status is `未/済/×`; `配布` is `未配布` / `配布済(v8)` (already distributed 読み・問題文) / `配布済` with `配布日`, stamped by Fable at sheet-output time on まるこ's instruction (`--distribute <id> <date>`); the human sheets for アイ are generated from the rows that are both `未` and `未配布`. 読み entries are per character for single-kanji surfaces and one entry per patch batch (with the word list) for multi-character words; 図 entries only for student-visible kinds. `教科` (算数/国語) is mandatory on every entry and Fable's sheet diffs are generated per subject. 国語 rows are NOT per generation path: they are per generated item (one question / one passage), added only when someone samples it and judges it, with the 7 columns 教科／経路／設問型／学年／record_id／判定(ok・suspect)／メモ (`review_queue_add.py --kokugo ...`); currently 0 rows. `tests/review_queue_integrity.js` validates the schema.

## Standing convention: paraphrase batches (言い換え便)

`tests/iikae_gate.js` compares every pattern's `sentence_templates` against the baseline in `tests/fixtures/iikae_baseline.json`. A rephrased pattern must keep the slot set and variant count, leave every field other than `sentence_templates`/`kaisetsu` byte-identical, stay within the grade's kanji table (`pattern_bank/kanji_grade.js`), differ from the old text (similarity < 0.8, length ratio 0.8–1.3), preserve numeric constants on transcription-exception patterns, and change only the 問題 lines of the generated golden output. After まるこ accepts a batch, run `node tests/iikae_gate.js --update` to move the baseline forward (never before acceptance).
