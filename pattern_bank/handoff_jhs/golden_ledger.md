...
### 2026-09-05 対称第1便: sym_figure受け皿(裁可j・逆算+合成+関門)——まるこ目視待ち
- **逆算(§1.2)**: 51行の転記ペアから写像(n,k)を導出。同一図形共有の行群ごとに整合解が一意: u01=例(n8,k6)/確認(n10,k0)/確認2(n14,k0)/練習(n14,k13) ・u02=例(n8)/確認(n10)/確認2・練習(n12)・u03=正五角形(n5,k2)/正六角形(n6・点)。**配線候補37行・不適合14行**(u01_029=補助点S未定義、u03_011/012=円、013/014=正五角形2個組合せ、015/019=表型→第2便、017/018/024=作図→域外(d)、028〜031=円交差/折り紙=図依存)。台帳=tests/fixtures/sym_figure_rows.json(転記記号のみ)。
- **sym_figure kind**(figure_builder): 格子座標(1目盛=20px・|x|,|y|≤6)・線対称=軸x=0/点対称=中心(0,0)。合成=半分を作って写す(§1.3): 右側頂点列(線)/半周(点)を「進行方向に右90/右45/左90の折れ」の乱歩で生成(初版の自由乱歩は内角制約でn12/14が2〜30%しか成立せず、折れ方式で7構造とも100/100)。幾何ガード=対称性(厳密)・単純多角形・辺方向0/45/90・内角{90,135,270}・格子域・**最小サイズ4×4目盛**(初版で極小図形が出たため追加)・点対称は中心が辺上に乗らない。正多角形(regular)・anchor固定(vertices)・補助点(extra_points: 辺の中点P)・方眼(grid)・軸ラベル(ア/イ)・中心ラベル(O)を受理。頂点ラベル=内角の二等分の外側±35°(line_set資産流用)・帰属自己検査。
- **関門43 sym_figure_vectors.js**: 10構造×seed100+正多角形=806照合(対称性・辺方向・内角・自己交差・サイズ・格子域を独立再計算、写像=転記ペア照合、帰属、決定性、契約4)。**関門39(等スケール)へsym_figure追加**(描画内角=幾何内角)。edge_set記号をA-H→A-N拡張(3実装・既存不変関門GREEN)・answer_domain "label"(記号ラベル答)を両エンジンへ。
- **全43関門GREEN・golden不変**。スモーク6枚(線対称n8/n14方眼・点対称n8/n12方眼・正五角形+P・正六角形)→まるこ目視待ち。

### 2026-09-05 対称第1便 配線(sym_figure 37行=13パターン)83→96・まるこ目視OK(スモーク6枚)
- **配線**: 単元×設問型で13パターン(単元混在を避け quiz生成の単元索引と整合): 線対称5(taiou_ten4/hen5/kaku4/seishitsu2/nagasa1)・点対称5(4/4/4/5/1)・多角形と対称3(ten1/hen1/seishitsu1)=行別レコード37(`symrows_*`・転記記号+写像(n,k)+grid/regular/anchor)。**skip 14行**(理由は関門台帳 fixtures/sym_figure_rows.json: 補助点S未定義1・円2・正五角形2個組合せ2・表型2→第2便・作図3→域外d・円交差/折り紙4=図依存)。
- **anchor固定2図**(数値答の行群): 線対称練習(n14,k13: FI−DK=6目盛×2cm=12cm)・点対称練習(n12・|EK|=14目盛×1cm・格子域7)。関門43(1b)で座標×1目盛cmから数値答・垂直・等長を再導出(37行・anchor6行)。
- **設問補正2件(報告)**: u03_021に「（1辺10cm）」を付与(5cmの根拠)・u02_021に「右の点対称な図形（点Oが対称の中心）で、」を付与(転記欠落)。辺答の表示は辞書順(転記「辺IH」=辺HI・「辺GF」=辺FG)。
- **golden md5: g06 b83b53f0 → 2ba2a568**(既存83件バイト不変・PASS 249→288 FAIL 0)。furigana残存0(増補なし)。corr-0007残差0。rationale+13(directives=D13・pedagogy=設計ノート・kaisetsuはFable後出し=backfill流儀)。pairs検証OK。**全43関門GREEN**。
- 台帳: 第2便=shapes型22+表型6+図なし3(設計書Fable)・grid作図18=域外(d)。

### 2026-09-05 ふりがな緊急パッチ(まるこ検収: 「行=い」誤爆)+関門44 single_char_contexts(1字登録の当たり文脈一覧)
- **修正**: 「行」=ぎょう(単独・表の行)に変更し、送り仮名付き「行き/行く/行っ/行け」=い を新設(3コピー同期・registered)。固定ベクター+4(行の合計/駅まで行く/列や行の数/学校へ行った)。全学年読み目検再実行=575パターン/34500標本 残存0。
- **関門44 tests/single_char_contexts.js**: 全学年バンクの本文/答え/kaisetsu/lexicon文字列を静的走査し、1字登録(179語)が当たった文脈(前後3字・数字N)を fixtures/single_char_contexts.json に固定。変化で差分表示+FAIL(人が読みを確認後 `--update`)。**初回採取で誤読2件を検出**→同便で増補: 算数=さんすう(「算=ざん」の誤爆・表族hyo_sansu本文)・千人/万人=にん(漢数字+人・kankyaku解説)。一覧更新済(当たり170語・文脈1458件)。
- corr-0038登録・pairs検証OK・**全44関門GREEN**・golden不変(読み層のみ)。

### 2026-09-05 ふりがな緊急パッチ第2弾(Fable読み上げ実測: 上/空)+恒久策(furigana_coverageに1字登録文脈一覧)
- 上=うえ(単独)・上がる/上がれ=あ(上がり/上がっ/上げは登録済)・空いて/空い/空く=あ(空=から維持)。上底/以上は複合表層で不変を確認。3コピー同期・registered 524→529・固定ベクター+6(列や行の数/それより上にある/はじめて上がる/空いている/空のはこ/上底)。全学年読み目検=残存0。
- **恒久策**: furigana_coverage(3)に「1字登録の当たり文脈一覧(surface→例文2件)」を出力し、関門44と同一ロジックで一覧差分を検出(コミットごと)。読みの正誤判定は人(アイさんのシート)。一覧更新済。corr-0039・全44関門GREEN・golden不変。
- **④(同便追加)**: 「がい数=がいすう」→「すう」(読み=漢字部分のみの規約)。かな+漢字キーで読みがかな部分から始まるものを機械検出=1件のみ。furigana_coverage(4)に辞書規約検査(かな始まり/かな終わりキー)を常設・ベクター+1(千の位までのがい数)。corr-0039へ追記。全44関門GREEN。

### 2026-09-05 解説backfill g04(Fable本文53本→51転写・2保留)+furigana増補
- **受領検証**: 53エントリ=pattern_id全一致・pedagogy無しの53と1:1。`{slot}`実在確認で**2件未解決**(g04_frac_convert_01/g04_frac_addsub_01の`{mixed}`=帯分数表示のスロットが未定義)→改変せず保留(要Fable: 既存名は num/den/num_sum のみ・帯分数表示は新規computed/表示派生が必要)。他51本を逐語転写(区切り線「---」の混入1件を除去)。
- **furigana**: 走査表30語のうち未登録13語+目検で検出した語=計41語増補(見当/直し/真下/真上/切り捨て/切り上げ/十/一/立て/楽/動き/単位/区切り/続き/文字/様子/名前/両はし/集まっ/止め/分母/分子/回り/向き/残り/折る/折っ/両側/結ん/結ぶ/方眼/反対側/必ず/正多角形/一つ/二つ/三つ/重なり・重なっ・重なる=かさ/回し・回す=まわ/区切る・区切っ=くぎ/間の角=あいだ)。関門44の文脈一覧が「重=おも(重なる)」「回=かい(回した)」「区」の誤読を検出→是正。「間の」登録が「4日間の」を巻き込む回帰をベクターで検出→「間の角」に限定。registered 529→574・固定ベクター+6(71)。辞書規約検査は漢字連2つ以上の表層(切り捨て等=全表層読み)を対象外に修正。
- **golden md5: g04 b7a48203 → 01ee1c7f**(解説行の追加のみ・解説行を除く出力はHEAD版と一致)。kaisetsu_resolve GREEN(JS=Python)・D10関門GREEN・全44関門GREEN。backfill在庫 493→442(g04残2)。

### 2026-09-05 対称第1便 kaisetsu 13本転写(Fable本文)
- 単元×設問型の13パターンへ割付(Fable指定の{p1}/{e1}/{k1}→{x1}・{ans}→{Y1}/{E1}・{gcm}→{cc1}の既存名へ機械置換)。点対称・長さ用に nagasa レコードへ x=E(直線EKの始点)を追加(スロットx1)。
- **golden md5: g06 2ba2a568 → 08d0f3ae**(解説行の追加のみ・解説行を除く出力は前版と一致)。kaisetsu_resolve GREEN。第2便設計書(shape_set)=裁可(k)待ち。

### 2026-09-05 対称第2便: shape_set受け皿(裁可k・カタログ+割当+関門)——まるこ目視待ち
- **shape_set kind**: 対称性カタログ20形(両方6: 正方形/長方形/ひし形/正六角形/十字/円・線のみ7: 正三角形/二等辺三角形/等脚台形/正五角形/矢印/T字/半円・点のみ3: 平行四辺形/Z字/風車形・どちらでもない4: 台形/L字/三角形/四角形。ハート形は輪郭再計算が困難なため未収録)。各形の線対称/点対称/軸本数を**輪郭座標から独立再計算**(重心を通る軸を0.5°刻み走査・180°回転)しカタログ値と一致(20/20)。
- **割当(§B.1)**: 記号ごとの要件(line/point/both/axes/axes_min/axes_max/finite・族限定tri/quad)を満たす形を重複なく決定的乱択(seed)。描画=セル84px・2〜4列・記号は左上・軸/中心は描かない・等スケール。
- **行台帳** tests/fixtures/shape_set_rows.json: 判別13行(線6・点7うち3行は中心書き込み部を域外d・両方2)+軸本数系2行(最少/最多)=15行。skip/転送12: 紙折り切り取り(u01_025)=域外候補・作図2=域外d・正多角形の可否4行=テキスト形(第2便)・表型2=table・図なし3=テキスト形。
- **関門45 shape_set_vectors.js**: カタログ再計算20+15行×seed100(真偽表=カタログ・正答集合の再導出=転記・重複/族/軸本数・帰属・決定性)+契約3=1523照合GREEN。全45関門GREEN・golden不変。
- スモーク3枚: shape_line.svg(ア〜ク・線対称のみ問う)/shape_point.svg(ア〜オ・点対称のみ)/shape_both.svg(三角形・四角形限定・両方)→まるこ目視待ち。

### 2026-09-05 shape_set r2(まるこ差し戻し: 紛らわしい反例・台形同居)
- 事実: 初版3枚の割当=線対称のみ(ア:L字 イ:矢印 ウ:Z字 エ:二等辺三角形 オ:正方形 カ:台形 キ:正五角形 ク:円)/点対称のみ(ア:正方形 イ:正三角形 ウ:風車形 エ:台形 オ:十字)/両方(ア:平行四辺形 イ:台形 ウ:長方形 エ:等脚台形 オ:四角形 カ:ひし形)。最良軸IoU=台形0.896・L字0.862・三角形0.922・四角形0.909(いずれも85%超=紛らわしい)。
- r2: ①等脚台形+台形の同居禁止(SS_EXCL) ②台形=直角台形/四角形=辺長差102%・非直角/三角形=辺長差91%/L字=腕太さ差(最良軸IoU 0.78/0.80/0.71/0.66) ③等脚系=座標鏡像(厳密) ④関門45(0b)紛らわしさ検査(格子IoU: どちらでもない形<85%・線対称=100%・点対称=回転100%・非点対称<85%)+②の数値検査。全1543照合GREEN・全45関門GREEN・golden不変。corr-0040登録。
- 再出力3枚(同名・同seed)→まるこ再目視待ち。

### 2026-09-05 対称第2便 配線(shape_set 15行=6パターン+テキスト形10行=2+表型2行=1)96→105・まるこ目視OK(r2)
- **shape_set 6パターン**(単元×設問型・行別レコード15): 線対称判別3(sen_hanbetsu)・軸本数最少/最多2(sen_jiku・label)・点対称判別4(tensym_hanbetsu)・三角形四角形の点対称2(tak_hanbetsu_ten)・両方2(tak_hanbetsu_ryo)・線対称+軸本数2(tak_sen_jiku・label)。答=記号集合(edge_setカタカナ・順不同)または転記文字列。**設問補正3件**: u02_009/u03_002/u03_005の「対称の中心Oをかき入れなさい」部を除去(作図=域外d・判別部のみ)。
- **テキスト形2パターン**: tak_text_01=正多角形の可否・軸本数4行(名前ラベル付きshape_setの図: 正三角形/正四角形/正五角形・正六角形/正八角形/正九角形=カタログへ正八角形/正九角形をnamed_only(判別の割当候補外)で追加)+図なし3行(tnc[2,3])。tak_text_en_01=円2行(circle図)+折り紙1行(u03_030・本文が状況を自立記述)。kaisetsu=設計書B.2/B.3を逐語。
- **表型1パターン**(tak_hyo_01・2行): table kind・空らん□・正方形行を与件・答は静的文字列。kaisetsu=B.2。
...
- (追記) 関門44一覧の目検で「分は分どうし」(時こくの解説)が ぶん と読まれるのを検出→「分どうし」「分は」=ふん を登録(「整数部分は」は部分が最長一致で不変)。ベクター+2(82)。

### 2026-09-05 backfill次学年=g05書き出し+概形棚卸し先出し
...
### 2026-09-06 approx_shape r2(まるこ差し戻し: 辺ラベルの距離)
- 辺ラベルのオフセット=担当辺の中点付近(±20%区間)での輪郭の外側はみ出しの最大+6px(辺ごと)。輪郭線をクリアランスから除外して直置き。audit/関門46に「辺ラベルと担当辺の距離≦18px」を追加(5枚とも8〜11px)。高さ/半径ラベルは現状維持。901照合GREEN・全46関門GREEN・golden不変。corr-0041。同名5枚再出力→再目視待ち。
- 所見②(u01_019): 本文に「円周率を3.1として」の指定なし→配線時に設問補正で付与(承認済み方針)。

### 2026-09-06 概形第1便 配線(approx_shape 9行=5パターン)105→110・まるこ目視OK(r2)
- **配線**: みなし図形別5パターン(rect2行/para2/trap1/tri3/circle1=行別レコード・寸法転記保存・答=面積式。円=転記答310保存+設問補正「（円周率を3.1として）」=所見②承認)。kaisetsu=設計書B.5を逐語({base_name}→行レコードbn1・{unit}→u1)。
- **skip/保留(報告)**: u01_007/008(円と正方形の倍=ます数え図)→第2便approx_grid候補、u01_024(平行四辺形−三角形の複合みなし・内訳未転記)、u01_027(図から寸法を読む)、**u01_009(図なし=転記本文に答え(①2②4③3)が併記され図依存)**→保留。設計書13行→配線9行。
- **設計補完(報告)**: 寸法未転記2行(u01_002 池=平行四辺形 b6×h3、u01_005 三角形 b6×h6)は転記答18を再現する寸法を設定。
- 関門46(1b)=生成器経由30本(答=みなし面積・図条件)追加=931照合GREEN。furigana+5(約=やく/池/方法/千葉県/参考)・registered 612・ベクター+2。rationale+5(D14)・pairs 589・corr-0007残差0。**golden md5: g06 41cc8a78 → ed171219**(既存105件バイト不変・PASS 315→330)。全46関門GREEN。
- 第2便(approx_grid 8+approx_solid 6)=Fable設計書後出し。

### 2026-09-06 解説backfill g05(Fable本文76本→76転写・保留0)
- 受領検証: 76=pattern_id全一致・未解決{slot}なし(文脈セット{ctx_*}含む)・区切り線混入1件(vol_unit_convert)除去。kaisetsu_resolve GREEN(JS=Python 1464標本)・D10関門GREEN。
- furigana: 走査表47語のうち未登録8語(倍数/約数/公約数/厘/高さ/底角/内のり/一の位)+目検12語(動かす/軽く/意味/最大公約数/言い方/限ら/囲む/入れ物 ほか)+**関門44一覧で検出「分かる」=ぶん→わ**(分かっ/分かり/分かる/分け)=計24語増補・registered 612→632・ベクター+3(87)。「分=ぶ」の当たり文脈: 歩合の解説は「N割N分N厘」形で複合規則により ぶ・単独「N分」は生成本文の文脈ヒント(unitSystem=歩合)で ぶ(表示層)。
- golden md5: g05 5f6ee326 → 0e8f8dfb(解説行の追加のみ・解説行を除く出力はHEAD版と一致)。backfill在庫 421→345(g05=0)。全46関門GREEN。

### 2026-09-06 概形第2便: approx_grid受け皿(裁可m・個数保存の探索固定・□=0.5こ・倍の2図)——まるこ目視待ち
- **approx_grid kind**: mode=blob(星形輪郭・格子中央基準・■+□÷2=target_areaに一致するseed/Rを決定的探索してanchor固定)／quarter_circle(半径r・厳密幾何)／circle_squares(倍の2図: 円+内接正方形■+外接正方形)。分類=各ますを5×5点サンプル(25点内側=■・1〜24=□)、描画と関門で同一導出。ラベル「1目もりNcm」は右下(バンク由来文字列)。
- **設計書からの差分(報告)**: §1.2「輪郭頂点を格子線から±0.08離す」は120頂点の星形曲線では成立確率≈0のため撤回(分類が点サンプルで頂点位置に非依存のため不要)。探索はキャンバス内条件のみ・R/amp 7段(±15%)。
- **anchor固定6行**(u01_001=17/003=25/004=25/013=40/014=36.5/023=22): 全て探索一致(sub探索1〜8で到達)。**skip 2行**: u01_017/018=四分円r=10の分類が5×5で■71/□14(頂点基準でも■69/□19)となり転記■69/□17と幾何的に一致しない(設計書§1.3の「転記個数が幾何的に不自然」)。倍の2図(u01_007/008)=circle_squares(答2倍/4倍は静的)。
- **関門47 approx_grid_vectors.js**: anchor6行の分類=台帳(独立実装)・自己交差/キャンバス/決定性・探索の決定性・倍の2図の比・四分円の契約値。全47関門GREEN・golden不変。
- スモーク: grid_blob25(問題図・■□なし)/grid_blob25_marks(参考: 分類の塗り)/grid_quarter(参考: 四分円の分類)/grid_circle_squares(倍の2図)→まるこ目視待ち。目視後に approx_solid→スモーク2枚→目視→14行配線。

### 2026-09-06 概形第2便: 倍の2図の判定規則確認+approx_solid受け皿——まるこ目視待ち(2枚)
- **倍の2図(circle_squares)**: 頂点基準の分類(外接正方形内のます)で、**境界上=内**なら内接■=0/□4→2倍・外接■4→4倍(転記再現)。**厳密内側(境界上=外)は内接2倍のみ再現し、外接は隅が境界上のため□4=2倍となり不一致**→契約値は境界上=内で固定(厳密は内接2倍を参考契約)。関門47に追加。四分円2行(017/018)=skip確定(頂点基準+厳密=■67/□19・境界=内=■69/□19・5×5=■71/□14、いずれも転記■69/□17と不一致)。
- **approx_solid kind**: base=cuboid(既存cuboidGeomの見取図・隠れ線なし・点線)/cylinder(上下楕円+側線・点線)。投影シルエット=見える頂点の凸包(円柱は楕円サンプル)に第1便の星形曲線を適用(amp0.10)。ラベル: w=正面底辺の外側(局所はみ出し+6px)・d=右奥行き辺・h=正面右辺の内側／円柱 r=上面半径線・h=右側線。体積=転記(w×d×h／r×r×3.14×h)。
- **関門48 approx_solid_vectors.js**: 行台帳5行(直方体010/011/012/020/028・028は小数寸法=静的答)×seed100=面積比[0.92,1.08]・内外はみ出し・自己交差・ラベル距離≦18px・体積=転記答・決定性+円柱契約+未知base=502照合GREEN。u01_026(自動車=複合直方体)=保留承認。全48関門GREEN・golden不変。
- スモーク: solid_cuboid(17×11×4cm)/solid_cylinder(r5×h8cm・素材に円柱行はなし=受け皿確認用)→まるこ目視待ち。目視後: 配線13行(grid 6+倍2+solid 5)+kaisetsu(§1.6/§2.4)→§3-3報告。

### 2026-09-06 概形第2便 配線(approx_grid 6+倍2+approx_solid 5=13行・3パターン)110→113・まるこ目視OK
- **配線**: g06_gaikei_grid_01(方眼6行=anchor固定・答は転記文字列「約N」・1目もり1{unit}ラベル)／g06_gaikei_bai_01(倍の2図2行=circle_squares・2倍/4倍静的・■/□=頂点基準境界上=内の分類値をレコード化)／g06_gaikei_solid_01(直方体5行・寸法転記・答は転記文字列・028は小数寸法=tnc宣言)。kaisetsu=設計書§1.6(■/□個数と面積は行レコード表示・倍の行は末尾一文追加)/§2.4を逐語({n_full}/{n_part}/{cell_cm}/{ans}/{unit}/{bn1}→行レコード名)。
- **skip確定/保留(報告)**: 四分円2行(017/018)=どの判定規則でも転記■69/□17と不一致・複合直方体(026)=保留承認・図なし(009)=保留。葉/手の形(013/014)は不定形輪郭で代替。設計書14行→配線13行。
- 関門47/48に(1b)バンク行検査(anchor分類=レコード・倍率契約・生成器経由の体積=答)追加=17/517照合GREEN。furigana+8(葉/一部/外側/辞書/浴そう/手/石けん ほか)・registered 638。rationale+3(D14)・pairs 592・corr-0007残差0。**golden md5: g06 ed171219 → 4d518400**(既存110件バイト不変・PASS 330→339)。全48関門GREEN。
- **概形(c10_u01)残余6行**: 009(図なし穴埋め)・017/018(四分円)・024(複合みなし)・026(複合直方体)・027(図読み)=保留。

### 2026-09-06 概形第2便§3-3受理(保留6行承認)+backfill次学年=g06書き出し+数直線便着手
- `kaisetsu_backfill_g06.md`(kaisetsu無し83パターン=g06 113のうち投入済み30を除く・g05と同形式)。
- 数直線便: 棚卸し先出し(11行全文引用=g04 9+g05 1+転記破損疑い1)を報告済。設計書(裁可n)は着弾待ち(Downloads未検出)。

### 2026-09-06 数直線便: number_line受け皿(裁可n・1kind3系統)+関門49——まるこ目視待ち(スモーク6図+段違い例)
- **number_line kind**: system=int(万/億/兆・label_fmt=big・base=1e4/1e8で絶対値化→「N兆N億N万」表示)/dec(0.1・0.01・0.001刻み・label_fmt=decN・記号の値は小目盛桁で表示)/frac(0〜整数をden等分・label_fmt=frac・整数/真分数/帯分数「Nと3/5」表示)。描画: 水平線+右矢じり・大目盛(長)+数値ラベル**上側**・小目盛(短)・記号は矢印↑を線の**下側**から立てて記号をその下(上下の領域分離で数値ラベルと矢印が衝突しない)。近接(18px未満)の記号は矢印長を段違い(y+26/y+48)にして2段。show_values=majors/ends/none・show_markers=false(作図型=矢印を出さず答は位置列挙)。
- **契約(例外)**: 範囲外marker・目盛上にないmarker・刻みが範囲/大目盛を割り切らない。
- **関門49 number_line_vectors.js**: 行台帳12図(転記10行=c01_004×3小問・c01_005×2・c01_007×2・c06_u01 004/009/013・c06_u02_002(compare)・g05_014(draw))。anchor=表示文字列が転記答・compare/draw行は値の大きい順の記号列=転記答。悉皆=12図×seed100(markers乱択: 目盛上・記号順保存・隣接≥2小目盛)で目盛座標/矢印座標=値の線形写像を独立再計算(誤差0)・記号帰属(ラベル中心xに最も近い鉛直要素=線の下側は矢印/上側は目盛)・間隔・数値ラベル非重なり・等スケール・決定性=1215照合GREEN。全49関門GREEN・golden不変。
- **設計書からの差分(報告)**: 帰属規則を「最近傍線分」から「水平距離最近傍(側別)」に変更(段違い矢印の胴が隣の記号をかすめる場合に線分距離では隣に帰属するため。数直線は全要素が鉛直なので水平距離で一意)。c06_u02_002は大目盛0.01×10本のため数値ラベルは両端のみ(show_values=ends)。
- スモーク: numline_smoke_{oku,man,dec2,dec3,compare,frac}.svg + numline_smoke.png(段違い例含む)→まるこ目視待ち。目視後: エンジン側(値スロット化+表示ヘルパ fmt_big/fmt_scaled の両エンジン追加)→配線10行(+skip 3行=素材待ち)→§3-3報告。

### 2026-09-06 解説backfill g06(Fable本文83本→83転写・保留0)
- 受領検証: 83=pattern_id全一致(投入済み30を除く残余83に完全対応)・未解決{slot}なし・区切り線混入1件(sym_judge末尾)除去。kaisetsu_resolve GREEN(JS=Python 9900標本)・D10関門GREEN。g06=113/113 kaisetsu完備。
- **「比／比べる」**: 既存表層 比べ=くら／比=ひ の最長一致で分離済み(速さを比べると→くらべると・等しい比→ひ・比の値→ひのあたい)。関門44の比=ひ当たり文脈9件は全て「比 N:」「比の値」=正。表層追加なし。
- furigana: 残存漢字12語(縮図上/次元/逆数/段階/文章/続く/番目/樹形図/相手/注意/角柱/柱状)+読み目検1周で誤読11語(分から=ぶん→わ・後ろ=あとろ→うしろ・の分は=ふん→ぶん(姉の分は)・組み合わせ=くみみあわせ・並べ方=ならべほう・数え=かずえ→かぞえ・見方=みほう→みかた・積ま=せき→つ・逆算・柱=はしら)=計23表層・**助数詞「人分」新設**(1人分=ひとりぶん・N人分=Nにんぶん: 数値+人が先に確定して「分は」=ふんに落ちる誤読の分離・3コピー同期)。registered 638→661・ベクター+4(91)・関門44一覧更新(--update・実読は数値+助数詞経路で全て正=□時□分から等は表層マスク上の見かけ差分)。
- **文言/読み申し送り(改変せず報告)**: ①g06_frac_time_sec「60分の15分…4分の1分」=分数の分と時間の分が連続し読みが ろくじゅっぷんのじゅうごふん/よんぷんのいっぷん に落ちる(分数を{f}表示「15/60」「1/4」に置くのが本筋・Fable判断待ち) ②g06_sym_judge「{judge1}です」→「線対称でも点対称でもあるです」(既存slot文の接続) ③既存読みの確認依頼(アイさんのシート): 多角形=たかっけい(教科書表記は たかくけい)・「2組の段」=ふたくみ(学級名なら にくみ)。
- golden md5: g06 4d518400 → bdc1781c(解説行の追加のみ・解説行を除く出力はHEAD版と一致・PASS 340不変)。pairs_design 592(83件のg06出力が解説込みに更新)。backfill在庫 345→262(g06=0・残=g01 46/g02 49/jhs 167)。全49関門GREEN。

### 2026-09-06 数直線便 配線(g04 5パターン=9行11レコード+g05 1パターン=1行)・まるこ目視OK(差分2件承認・上下配置維持)
- **エンジン側**: 表示ヘルパ3種を両エンジンへ(fmt_big=「N兆N億N万」分解・fmt_scaled=整数×10^exp固定小数・order_desc=大きい順の記号列)・helpers_test_vectors +3種22件(JS/Python 31種225件一致)。number_lineのmarkerは`k`(小目盛index)を受理(整数スロットで位置を持てる=floatをenvに入れない)。
- **配線**: g04_numline_big3_01(c01_u02_004の3小問=3レコード)／big2_01(007の2小問+005(2)=3レコード・005(2)は転記順②<①をsg=-1で保存)／dec3_01(c06_u01 004/009/013・単位m/km付き答)／hikaku_01(c06_u02_002=mode:compare・答=order_desc→常に「イ、ウ、ア」・設問の答え漏れ「イ、ウ、アの順に」を「ア、イ、ウの数を大きい順に書きなさい」に補正・「並べ」はg6漢字のため「書き」)／kinyu_01(c01_u02_005(1)=作図型・ア/イの値を本文で与え答=「0から小さい目もりkこ分右」の位置列挙・矢印なし)／g05_numline_frac_kinyu_01(g05_c06_u03_014=draw型・4数の位置を行レコード・答=order_desc「2と3/5、8/4、1.6、0.4」・anchor固定=数の表記が本文に実在)。行別レコード numline_rows(11)/numline_frac_rows(1)。可変化=位置k乱択(転記順保存・隣接≥2・第1記号は大目盛上を避けて解説「こ分」が成立=制約first_off_major)。制約は行別上限を`[..][j1]`のリスト添字で表現(制約評価時のenvは数値スロットのみ)。
- **kaisetsu**: 設計書§6の3本を逐語({major_disp}{minor_disp}{n_from}{k}{ans}{den}→mj_disp/mi_disp/nf_disp/kk/ad1_disp・an1_disp/den1)。decのn_fromは大目盛精度(2.8・7.46)。
...
### 2026-09-06 解説backfill g06 申し送り2件の反映(まるこ回答)
- ①「4分の1分」=分数表示化可: 本文は既に「{s1}/60分」「{res1}分」の分数表示で、読みが「ぶん」に落ちていた側を是正。**読みエンジンに「分数(帯分数)+単位」規則**を追加(直後に助数詞/単位が続くとき分子を助数詞読みで結合: 1/4分=よんぶんのいっぷん・15/60分=ろくじゅうぶんのじゅうごふん・3/4m=よんぶんのさんメートル・3/4L=…リットル)。3コピー同期(pattern_bank/assets_global/テンプレpg-engine)・ベクター+3(95)・関門44一覧不変。
- ②g06_sym_judge_01 kaisetsu: 「{judge1}です」→「答えは「{judge1}」です」(改変承認)。golden md5: g06 bdc1781c → 4d141235(sym_judge解説行のみ・前記4a5a1a3cは誤記)。全50関門GREEN。
- 監視: コミット前の全関門ループで number_line_vectors が1回だけ赤(内容未捕捉)→直後の単独再実行21回は全てGREEN(1575構成)。(2b)の乱択生成(makeProblem)に依存する部分の稀な棄却失敗の疑い=再発時はログ捕捉して原因特定。

### 2026-09-06 number_line_vectors 単発赤の根因特定(corr-0042・まるこ指示)
- **根因**: 比較行(1.4〜1.5=100小目盛=3px/目盛)で3記号が3連近接(例 k=71/73/75)すると、2段交互(0/1/0)の段違い規則で1番目と3番目が同段12px差になり記号ラベルが重なる(label_overlap)。6パターン×4000seedの探索で hikaku_01 のみ 2/4000 再現(seed298/857)。関門(2b)は30本/回の無seed乱択だったため約1.5%/回で赤=「約20回に1回」と一致。
- **修正**: 段違い規則を貪欲割当(左から順に、同段の既配置記号と18px以上離れる最小の段・最大3段・矢印長 y+26+22×段)に変更。関門(2b)は Math.random を seed付き mulberry32 に差し替えて決定化(200本/パターン・失敗時は seed/markers/env を出力)+回帰ベクター(2c)追加=3617構成。再探索36000構成で失敗0・全50関門ループ3周GREEN。golden不変。

### 2026-09-06 等分図便 配線(g02 2+g03 1=新パターン・g06 4=既存パターンへ図追補)・まるこ目視OK(差分3件承認)
- **配線**: g02_tobun_nuri_01(c51_u01_003=作図型・den∈{2,3,4,8}・num=1・図型は行レコード円/長方形/テープ・答=「{den}等分した{num}こ分に色をぬる」列挙)／g02_tobun_yomi_01(c51_u01_004=読み取り型・den∈{2,3,4,8}・num<den・既約(Code追加: 補完値1/2・1/3・1/4・1/8・3/4・3/8に整合)・答={num}/{den}機械採点)／g03_tobun_draw_01(c26_u01_002/007/013=5小問・テープ1m/1L・den∈[2,9](設計書§4)・num<den・013の6/11は乱択域外=anchor台帳)／g06 c03 word系4(x_int=col_unit・div_int=1L・x_frac=面積図・div_frac=補助図)に equal_parts rect 2方向の figure_params を追補。行別レコード tobun_rows(g02 5/g03 5)。
- **kaisetsu**: 設計書§6を逐語(g02 read→tobun_yomi・g03 draw→tobun_draw と g02 nuri(g02作図型固有の本文は無いためg03 drawを流用=「等分」は3年配当・要確認)・g06面積図→x_frac_word_01の既存backfill本文を置換。x_int/div_int/div_frac は既存本文維持=面積図本文は×分数専用のため)。
- **表記(報告)**: 転記「塗った」「次の」はg02配当外漢字のため「ぬった」「つぎの」。g02の答「{den}等分した…」は設計書§3の様式を逐語(「等」は3年配当=要確認)。
- **skip**: g03_c27_u01_004(設問・答とも未転記)。**拡張候補9行(台帳のみ・裁可o-5)**: g02_c51_u01_002/005/006/007・g03_c26_u01_004/005/009/010/015(has_figure=FALSEだが図読み前提。ア・イ記号付きはnumber_line分数系統の候補)。
...
### 2026-09-06 line_bar_combo(裁可e・方針確定): xy_graph mode=combo 受け皿+関門51——まるこ目視待ち(スモーク1枚)
- **xy_graph mode=combo**: 折れ線(左軸・既存polylineの1-2-5刻み)+棒(右軸・1-2-5自動刻み plNiceTick・hi2=最大値切り上げ・目盛≦10本)。x軸はカテゴリ位置((i+0.5)·W/n)で棒がプロット内に収まる。題→凡例(線見本+棒見本+系列名=fp由来)→軸題の3段。polylineは非破壊(独立分岐)。
- **関門51 xy_combo_vectors.js**: anchor(c02_u02_002=気温/降水量の極値4点: 8月29度・1月15度・7月250mm・3/11/12月10mm)+悉皆(seed付き合成系列300・右軸最大値を60〜2400で振り1-2-5刻みを網羅)+(2b)バンク配線(配線後に有効)+契約4。出力SVGから独立再計算: 棒の高さpx=値の線形写像(右軸)・折れ線の点px(左軸)・棒中心=カテゴリ位置・右軸刻み∈{1,2,5}×10^k・極値の一意性・棒minの同値月集合・等スケール・決定性=305構成GREEN。全51関門GREEN・golden不変。
- スモーク: combo_smoke_kion.svg / combo_smoke.png(anchor系列=極値4点固定・他月合成)→まるこ目視待ち。目視後: g04_line_bar_kion_01 配線(極値固定・他月乱択・答保存・kaisetsuはFable本文待ち=backfill在庫+1)→§3-3報告。#2(004)/#3(006)=skip素材待ち。

### 2026-09-06 解説backfill g01+g02(Fable本文95本→95転写・保留0)
- 受領検証: 95=pattern_id全一致(g01 46/g02 49)・未解決{slot}なし・区切り線混入なし。g01は分かち書きのまま逐語。kaisetsu_resolve GREEN(JS=Python 13020標本)・D10関門GREEN。小学全学年(g01〜g06)のkaisetsu完備=backfill在庫 262→167(残=jhs 167)。
- **「分」の2読み(Fable申し送り・関門44確認)**: 「{n1}分の一」は読みエンジンの「N分のM」規則をM=一(漢字)まで拡張して ぶんのいち・「何分」は既存の助数詞経路で なんぷん。副作用の是正2件: ①「午前10時35分の30分後」が「35分の30」で分数規則に当たった→直前が「時」なら時こく(分=ふん)として除外 ②時こく計算の「分」(のこりが分になります/のこりの分を/時こくの分に)が ぶん に落ちていた→表層 が分に・の分を・時こくの分=ふん(g03/g05の「までの分を数え」「その分をたして」も同時に ふん に是正)。
- furigana: 誤読1語(くり下げ=くりしたげ→くりさげ: 表層「下げ」=さ)+上記3表層=計4・registered 666・ベクター+5(104)・関門44一覧更新。残存漢字0。
- golden md5: **g01 3677308f → f8f8be4a**(解説138行の追加のみ)・**g02 ad3e4c64 → 1d8d2d34**(解説行の追加のみ・解説行を除く出力はHEAD版と一致)。全51関門GREEN。

### 2026-09-06 line_bar_combo スモーク差し戻し(x軸ラベル重なり)→x軸ラベル圧縮規則(xy_graph共通)+関門(2c)——再目視待ち
...
### 2026-09-07 graph_choice 自己差し戻し(まるこ設計側): 全4面を折れ線に(棒での描き分け廃止)——再目視待ち／検収台帳: 配布済印取り消し・配布日欄
- **graph_choice**: kindはtopic群から導出せず全面line固定(種類別の題材も点をつないだ折れ線)。判別=横軸が時間か種類かのみ。audit: not_line/group_mismatch/dup_topic・正答の再導出=時間軸群の面。関門52の「折れ線/棒面数の照合」を「4面とも折れ線(polyline=4・棒rect=0)・正答面=時間軸群・非正答面=種類別群」に差し替え=504構成GREEN((2b)配線済みパターンも同条件で200本)。同名2枚再出力(graph_choice_smoke_004/007.svg)。
- **検収台帳**: 「配布済(v8/v4)」印を取り消し全827件=未配布・欄「配布日」新設(配布時にまるこが `review_queue_add.py --distribute <id> <日付>` で記入=配布済)。関門53: 配布∈{未配布,配布済}・配布日は配布済のときのみ日付。全53関門GREEN・golden不変。
- 再補正(まるこ): 読み(1字182)・問題文(既存592)は「配布済(v8)」印のまま・図(37)と本日新設分(問題文11・読みパッチ5)は未配布。配布はまるこの指示でFableがシート出力時に「配布済」印と配布日を入れる運用(関門53: 配布済のときのみ日付必須・配布済(v8)は日付なし可)。

### 2026-09-07 graph_choice便 クローズ(r2再目視OK・§3-3再提出)
- 配線内容は 0b82240 のまま有効(図のみr2=全面折れ線)。g04_graph_choice_01(c02_u01_004/007)・golden g04 8a4578fa・関門52=504構成GREEN・検収台帳: 図graph_choice(未・まるこ)は目視済につき「済」。kind裁可(e)候補は全件クローズ。次=jhs backfill(章ごと着弾)。

### 2026-09-07 解説backfill jhs第1便(c01〜c06・Fable本文67本→67転写・保留0)
- 受領検証: 67=pattern_id全一致(c01 21/c02 15/c03 5/c04 6/c05 7/c06 13)・未解決{slot}なし・区切り線混入なし。kaisetsu_resolve GREEN(JS=Python 15090標本)・D10関門GREEN。**表示派生の重点確認**: {ans_disp}(負数「−16」「−11/45」)・{cd1}{td1}{sg1}{ac1}等の係数/符号表示は本文テンプレと同じ解決で全67本が数式として整合(読み目検1周で確認)。backfill在庫 167→100(残=jhs c07〜c18)。
- furigana: 残存漢字53パターン→jhs語彙29語(負の数/正の数/負/正/符号/絶対値/係数/同類項/項/各項/移項/代入/両辺/分配法則/単項式/乗除/指数/性質/成り立/加減法/基本/双曲線/近づく/端点/比例定数/格子点/防げ/以外/外の)+読み目検で誤読6語(変形=か+かたち→へんけい・解い=かい→と・年後=ねんあと→ねんご・x人=ひと→にん・x分=ぶん→ふん・x上=うえ→じょう・消え=け→き)=計35表層・registered 705・ベクター+5(111)・関門44一覧更新(67文脈=正)。**読み規則追加: 負の分数**(−11/45=マイナスよんじゅうごぶんのじゅういち・従来は「マイナスじゅういち/よんじゅうご」)・3コピー同期。
- golden md5(解説行の追加のみ・解説行を除く出力はHEAD版と一致): c01 76dbe64c→c40a8d04・c02 082b2c3f→f8a83493・c03 6d8e6ede→cd3c1244・c04 b221d05f→bb937a30・c05 a571bf63→8f560945・c06 00ba3cf0→7bfa122b。検収台帳: 読みパッチ1件追記(未・アイ)。全53関門GREEN。

### 2026-09-07 解説backfill jhs第2便(c07〜c13・Fable本文56本→56転写・保留0)
- 受領検証: 56=pattern_id全一致(c07 12/c08 8/c09 11/c10 5/c12 11/c13 9)・未解決{slot}なし。kaisetsu_resolve GREEN(JS=Python 16770標本)。**{C1_choice}(合同条件の記号)はア/イ/ウに解決**(c13 jouken_sss/sas/asa=答え表示と同一の表示派生)。{lo1_disp}{ac1}等も本文と同じ解決で56本が整合(読み目検1周)。backfill在庫 100→44(残=c14〜c18)。
- furigana: 残存漢字→jhs語彙17(真ん中/裏/帰り/最小値/最大値/最小/最大/消去/後半/着く/達する/上限/起こり/対頂角/錯角/外角/言え)+読み目検で誤読16表層(分かれ=わ・x分後=ふんご・線上/グラフ上/直線上/一直線上=じょう・角形=かくけい・同位角=どういかく(おなくらいかく誤爆)・同側内角・内角=ないかく・の間に/間にない=あいだ(「5分間に」=ふんかんは維持=表層を「間に」から限定)・階級値=かいきゅうち・出方=でかた・取り出さ・（大，小）=だい，しょう・た分は=ぶん(こえた分は)・小数第1〜3位=だいNい)=計37表層・registered 742・ベクター+6(117)・関門44一覧更新。**読み規則追加: 第N位=だいNい**(小数第は表層で吸収)・3コピー同期。
- **D10-2(表記規則の改訂・Code検出)**: 関門41が解説「この点(x＝8)を通る」を「ラベル こ＋の点」として誤検出→「ラベル＋の＋(角|点|直線)」型から こ を除外(指示語)。JS/Python/インライン4経路を同時更新(drift GREEN)・D10_表記規則.mdに改訂記録。
- golden md5(解説行の追加のみ・解説行を除く出力はHEAD版と一致): c07 02ab26f9→ea8bfb2a・c08 8c1197f7→97252d60・c09 7bdf4082→128d2715・c10 9427e7a3→5f4460ca・c12 56e9ee03→4767badd・c13 c1814bc2→f1f33608。検収台帳: 読みパッチ1件追記。全53関門GREEN。

### 2026-09-07 解説backfill jhs第3便(c14〜c18・Fable本文44本→44転写・保留0)=最終便・**backfill在庫0(全602+1パターンkaisetsu完備)・取り組みクローズ**
- 受領検証: 44=pattern_id全一致(c14 15/c15 9/c16 6/c17 7/c18 7)・未解決{slot}なし。kaisetsu_resolve GREEN(JS=Python 18090標本)。**表示派生の確認**: π表示{V1_pi}{S1_pi}(180π・78π・(64/3)π・(256/3)π)・根号{sd1}{sdD}(3√11・8√7・x＝3±4√3・x＝(7±√21)/2・±√47)・辺集合{E1_edges}(辺CG、辺DH、辺EH、辺FG)が本文と同じ解決で44本が整合(読み目検1周)。
- furigana: jhs語彙21(外に/定数項/共通因数/素因数分解/因数/周/中学/遠い/除い/集まる/互いに/最後/共通/平方数/逆に/平方根/放物線/開か/一定/他の/確かめ)+読み目検で誤読5表層(空間=からかん→くうかん・重解=おもかい→じゅうかい・出す/出せ=だ・側面積/側面=そくめん)=計26・registered 768・ベクター+4(121)・関門44一覧更新(**解=かい(解の公式/解は2つ/x＝5が解)と解い=と(解いて)の分離を確認**・表面積=ひょうめんせき)。
- golden md5(解説行の追加のみ・解説行を除く出力はHEAD版と一致): c14 83401fee→2fcfbb89・c15 a1e38dba→4c91eb6b・c16 2d8f37b7→7551607e・c17 1df8cea7→e08a49a2・c18 5bda8385→5d12fdaf。検収台帳: 読みパッチ1件追記。全53関門GREEN。
- **クローズ**: 解説backfill(e-9由来)は小学g01〜g06+中学c01〜c18の全パターンで完了(在庫0)。残る取り組み=言い換え便(関門実装待ち)・素材待ち行(skip台帳)・アイさんの戻り(検収台帳の未・未配布)。
